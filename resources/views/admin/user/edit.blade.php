                    ...
                    <div class="card-body">
                        ...
                        @if($xver->business_type=="Registered Business")
                            ...
                            <p>{{__('Reg no')}}: {{$xver->reg_no}}</p>
								...
								@endif
							 @if($xver->DOC_CRD != null)
								<a href="{{url('/')}}/asset/profile/{{$xver->DOC_CRD}}">{{__('Corporate Registration Document')}}</a>                            <br><br>
								@endif
						
								@if($xver->DOC_RDAA != null)
									<a href="{{url('/')}}/asset/profile/{{$xver->DOC_RDAA}}">{{__('Resolution of Directors           authorizing Application')}}</a><br><br>
								@endif
									 @if($xver->DOC_CID != null)
									<a href="{{url('/')}}/asset/profile/{{$xver->DOC_CID}}">{{__("
		Corporate ID (2 pieces gov't issued) of Directors or officers of the company")}}</a><br><br>
									@endif
							@if($xver->DOC_IS != null)
								<a href="{{url('/')}}/asset/profile/{{$xver->DOC_IS}}">{{__("2 invoices from suppliers less than 3 months old")}}</a><br><br>
								@endif
							@if($xver->DOC_BVC != null)
								<a href="{{url('/')}}/asset/profile/{{$xver->DOC_BVC}}">{{__("A Bank void check with the cancelled written across")}}</a><br><br>
								@endif
							@if($xver->DOC_MRD != null)
								<a href="{{url('/')}}/asset/profile/{{$xver->DOC_MRD}}">{{__("
	Most recent processing statements up to 6 months")}}</a><br><br>
								@endif
							@if($xver->DOC_ABF != null)
								<a href="{{url('/')}}/asset/profile/{{$xver->DOC_ABF}}">{{__("A resolution of the Company stating that the Co is in good standing and has no claims against it from the major card carriers by an issuing or acquiring Bank for fraud")}}</a><br><br>
								@endif
                        @else
