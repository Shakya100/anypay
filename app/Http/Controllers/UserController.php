...
class UserController extends Controller
{
    ...
    //Invoice
        ...
        public function addinvoice()
        {
			...
			$data['invoiceno']=Invoice::latest()->first();
        ...
        public function Paidinvoice($id)
        {
            ...
            if($up_mer->balance>$charge || $up_mer->balance==$charge){
                ...
                $data->save();
                //Charges
                $charges['user_id']=$data->user_id;
                $charges['ref_id']=$data->ref_id;
                $charges['amount']=$data->total*$set->invoice_charge/100+($set->invoice_chargep);
                $charges['log']='Charges for invoice #' .$data->ref_id;
    ...
    //Settings
        ...
        public function no_kyc()
        {
            ...
            $set=Settings::first();
            $user = User::find(Auth::guard('user')->user()->id);
			$data['usersData'] = User::whereid($user->id)->first();
            $data['site']=$set->site_name;
        ...
        public function submitcompliance(Request $request)
        {
			...
			if ($request->hasFile('DOC_CRD')) {
                $image = $request->file('DOC_CRD');
                $filename = 'DOC_CRD'.time().'.'.$image->extension();
                $location = 'asset/profile/' . $filename;
                if ($com->DOC_CRD != null) {
                    $path = './asset/profile/';
                    $link = $path . $com->DOC_CRD;
                    if (file_exists($link)) {
                        @unlink($link);
                    }
                }
                Image::make($image)->save($location);
                $com->DOC_CRD=$filename;
            }      
			if ($request->hasFile('DOC_RDAA')) {
                $image = $request->file('DOC_RDAA');
                $filename = 'DOC_RDAA'.time().'.'.$image->extension();
                $location = 'asset/profile/' . $filename;
                if ($com->DOC_RDAA != null) {
                    $path = './asset/profile/';
                    $link = $path . $com->DOC_RDAA;
                    if (file_exists($link)) {
                        @unlink($link);
                    }
                }
                Image::make($image)->save($location);
                $com->DOC_RDAA=$filename;
            }  
			if ($request->hasFile('DOC_CID')) {
                $image = $request->file('DOC_CID');
                $filename = 'DOC_CID'.time().'.'.$image->extension();
                $location = 'asset/profile/' . $filename;
                if ($com->DOC_CID != null) {
                    $path = './asset/profile/';
                    $link = $path . $com->DOC_CID;
                    if (file_exists($link)) {
                        @unlink($link);
                    }
                }
                Image::make($image)->save($location);
                $com->DOC_CID=$filename;
            }  
			if ($request->hasFile('DOC_IS')) {
                $image = $request->file('DOC_IS');
                $filename = 'DOC_IS'.time().'.'.$image->extension();
                $location = 'asset/profile/' . $filename;
                if ($com->DOC_IS != null) {
                    $path = './asset/profile/';
                    $link = $path . $com->DOC_IS;
                    if (file_exists($link)) {
                        @unlink($link);
                    }
                }
			...
			if ($request->hasFile('DOC_BVC')) {
                $image = $request->file('DOC_BVC');
                $filename = 'DOC_BVC'.time().'.'.$image->extension();
                $location = 'asset/profile/' . $filename;
                if ($com->DOC_BVC != null) {
                    $path = './asset/profile/';
                    $link = $path . $com->DOC_BVC;
                    if (file_exists($link)) {
                        @unlink($link);
                    }
                }
                Image::make($image)->save($location);
                $com->DOC_BVC=$filename;
            }  
			if ($request->hasFile('DOC_MRD')) {
                $image = $request->file('DOC_MRD');
                $filename = 'DOC_MRD'.time().'.'.$image->extension();
                $location = 'asset/profile/' . $filename;
                if ($com->DOC_MRD != null) {
                    $path = './asset/profile/';
                    $link = $path . $com->DOC_MRD;
                    if (file_exists($link)) {
                        @unlink($link);
                    }
                }
                Image::make($image)->save($location);
                $com->DOC_MRD=$filename;
            } 
			if ($request->hasFile('DOC_ABF')) {
                $image = $request->file('DOC_ABF');
                $filename = 'DOC_ABF'.time().'.'.$image->extension();
                $location = 'asset/profile/' . $filename;
                if ($com->DOC_ABF != null) {
                    $path = './asset/profile/';
                    $link = $path . $com->DOC_ABF;
                    if (file_exists($link)) {
                        @unlink($link);
                    }
                }
                Image::make($image)->save($location);
                $com->DOC_ABF=$filename;
            } 
    ...
    //Bank functions 
        ...
        {  
            ...
            if($set->stripe_connect==1){
				...
				$request->validate([
				'pay_acct_no'=>'required_without:net_acct_no',
				'pay_acct_name'=>'required_without:net_acct_name'
				]);
                $data['pay_acct_no']=$request->pay_acct_no;
                $data['pay_acct_name']=$request->pay_acct_name;
				$data['net_acct_no']=$request->net_acct_no;
                $data['net_acct_name']=$request->net_acct_name;
